...
function nextPrev_1() {
	...
	const getCombinedSharePercentage = Array.from(shareInputs).reduce((total, input) => total + parseFloat(input.value || 0), 0);
	...
	if (getLocalStorage && !getLocalStorage.legelRepresentative && getLocalStorage.manager && Number(getCombinedSharePercentage == 100)) {
		x[0].style.display = 'none'
	...
	// && Number(document.getElementById('firstSharehoderPercentage').value == 100)
	if (getLocalStorage && getLocalStorage.legelRepresentative && getLocalStorage.manager) {
		localStorage.setItem(window.location.href, JSON.stringify(
			...
			{
				...getLocalStorage,
				per: Number(getCombinedSharePercentage)
			}
		))
	}

	if (Number(getCombinedSharePercentage == 100) &&
...
function nextPrev_2() {
	...
	getLocalStorage.legelRepresentative = true;
	getLocalStorage.data_.shareholder.push({
		...getLocalStorage.a,
		a
	})
	localStorage.setItem(window.location.href, JSON.stringify(getLocalStorage))
